...
- **📋 案件管理** — プロジェクト進捗トラッキング・ステータス管理（計画中 / 進行中 / レビュー中 / 保留 / 遅延）・担当者アサイン・クライアント名管理
- **✅ タスク管理** — 優先度（低 / 中 / 高）・期限管理・ステータスフィルタ（TODO / 進行中 / レビュー / 完了）・モーダルで即時作成
...
- [x] AWS CDK インフラ実装（6 スタック）
- [x] Lambda 関数実装（Node.js / Python）
- [x] LangGraph Agent 実装（SSE ストリーミング対応）
- [x] CI/CD パイプライン構築（GitHub Actions OIDC）
- [x] コードベース品質改善（SSE 構造統一・型定義修正・権限チェック強化）
- [ ] セキュリティテスト・本番デプロイ
